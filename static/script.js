...
let lastFocusedInput = null;  // Track which input field is focused
...
function quickWithdraw(amount) {
    // Show the withdraw screen first
    showScreen('withdraw-screen');
    
    // Set the amount
    document.getElementById('withdraw-amount').value = amount;
    
    // Call withdraw after a small delay to ensure screen is shown
    setTimeout(() => {
        withdraw();
    }, 100);
}
...
function loadAccountInfo() {
    ...
    .then(data => {
        if (data.status === 'success') {
            ...
            document.getElementById('info-account').textContent = data.account_no;
            document.getElementById('info-balance').textContent = `₹ ${data.balance.toLocaleString('en-IN')}`;
        }
    })
    .catch(error => console.error('Error:', error));
}
...
function initializeFocusTracking() {
    ...
    inputs.forEach(input => {
        ...
        input.addEventListener('blur', function() {
            // Keep the reference for a moment in case keypad is clicked
            setTimeout(() => {
                if (document.activeElement !== this) {
                    // Restore focus to this input if another element takes focus
                }
            }, 10);
        });
    });
...
function initializeKeypad() {
    const keypadButtons = document.querySelectorAll('.keyboard-panel .key');
    
    keypadButtons.forEach(button => {
        button.addEventListener('click', function() {
            handleKeypadInput(this.textContent.trim());
        });
    });
}

function handleKeypadInput(value) {
    // Use the last focused input or find one in the active screen
    let targetInput = lastFocusedInput;
    
    const activeScreen = document.querySelector('.screen-view.active');
    if (!activeScreen) return;
    
    // If no input was previously focused, try to find one in current screen
    if (!targetInput || !isInputVisible(targetInput)) {
        const inputInScreen = activeScreen.querySelector('input[type="text"], input[type="password"], input[type="number"]');
        if (inputInScreen) {
            targetInput = inputInScreen;
        } else {
            return;
        }
    }
    ...
    if (value === 'CLEAR') {
        ...
        handleBackspace(targetInput);
    } else if (value === 'OK') {
        handleOK(screenId);
    } else if (value === 'CANCEL') {
        handleCancel();
    } else if (!isNaN(value)) {
        // It's a number, append to input
        if (targetInput && (targetInput.tagName === 'INPUT')) {
            // For password/number inputs with max length
...
function isInputVisible(input) {
    // Check if input is in the currently active screen
    const activeScreen = document.querySelector('.screen-view.active');
    if (!activeScreen) return false;
    return activeScreen.contains(input);
}
...
function handleOK(screenId) {
    // Trigger the appropriate action based on current screen
    switch(screenId) {
        case 'login-screen':
            login();
            break;
...
function handleCancel() {
    ...
    const activeScreen = document.querySelector('.screen-view.active');
    if (activeScreen && activeScreen.id !== 'login-screen' && activeScreen.id !== 'menu-screen') {
        showScreen('menu-screen');
    } else if (activeScreen && activeScreen.id === 'login-screen') {
        // Clear login form
        document.getElementById('card-no').value = '';
        document.getElementById('pin').value = '';
        lastFocusedInput = null;
